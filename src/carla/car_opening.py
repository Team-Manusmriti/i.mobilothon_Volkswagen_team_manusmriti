# Nissan Patrol & doors + eCall + Vehicle Securement + High-G Impact + Rollover & Medical + Head Pose Analysis + Traffic Analysis + Anomaly + Driving Patterns + Crash Detection
import carla
import random
import time
import os
...
def main():
    ...
    DRIVER_INACTIVITY_THRESHOLD_SECONDS = 3.0
    ROLLOVER_THRESHOLD_DEGREES = 90.0
    HIGH_G_IMPACT_THRESHOLD = 2000.0
    
    vehicle_state = {'state': "NORMAL", 'event_type': "None"} 
    
    ecall_sent = False
    # --- NEW: Flag to ensure doors open only once ---
    doors_opened = False
    ...
    try:
        ...
        # --- MODIFIED: Spawn a "Demo-Friendly" Vehicle ---
        try:
            # The Nissan Patrol is known to support door opening
            vehicle_bp = blueprint_library.find('vehicle.nissan.patrol_2021')
            print("Found 'vehicle.nissan.patrol_2021', door opening enabled.")
        except IndexError:
            # Fallback if that vehicle isn't available
        ...
        while running:
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        ...
                        if vehicle_state['state'] != "NORMAL":
                            ...
                            ecall_sent = False
                            # --- NEW: Reset doors_opened flag ---
                            doors_opened = False
                            # (Note: We don't close the doors, just reset the flag)
            ...
            # --- Securement & Control Logic (Phase 2 & 3) ---
            ...
            if vehicle_state['state'] == "EMERGENCY_STOP":
                ...
                # --- NEW: Feature 4.2: Open Doors (Once) ---
                if not doors_opened:
                    print("--- VEHICLE SECURED: Opening doors. ---")
                    try:
                        # This command opens all doors
                        vehicle.open_door(carla.VehicleDoor.All)
                        doors_opened = True
                    except AttributeError:
                        # This will happen if we fell back to the Model 3
                        print("--- (Vehicle does not support door opening feature.) ---")
                        doors_opened = True # Set flag anyway to prevent retrying
        ...
        if 'vehicle' in locals() and vehicle.is_alive:
            vehicle.set_light_state(carla.VehicleLightState.NONE)
            # --- NEW: Close doors on cleanup ---
            try:
                vehicle.close_door(carla.VehicleDoor.All)
            except AttributeError:
                pass # Vehicle didn't support it anyway
